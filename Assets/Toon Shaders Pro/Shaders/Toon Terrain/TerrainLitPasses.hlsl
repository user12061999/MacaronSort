...
#else
    #define TERRAIN_SAMPLE SAMPLE_TEXTURE2D
#endif
...
#include "ToonTerrainFunctions.hlsl"
...
float4 stochasticTexturing(texture2D tex, sampler samplerState, float2 uv)
{
    float4x3 BW_vx;
    ...
    return  mul(SAMPLE_TEXTURE2D_GRAD(tex, samplerState, uv + hash2D2D(BW_vx[0].xy), dx, dy), BW_vx[3].x) +
            ...
            mul(SAMPLE_TEXTURE2D_GRAD(tex, samplerState, uv + hash2D2D(BW_vx[2].xy), dx, dy), BW_vx[3].z);
}
...
void InitializeInputData(Varyings IN, half3 normalTS, out InputData inputData)
{
    ...
    inputData.positionCS = IN.positionCS;
        ...
        inputData.tangentToWorld = half3x3(-IN.tangentWS.xyz, IN.bitangentWS.xyz, IN.normalWS.xyz);
...
void NormalMapMix(float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, inout half3 mixedNormal)
{
        ...
        half3 nrm = half(0.0);
        nrm += splatControl.r * UnpackNormalScale(TERRAIN_SAMPLE(_Normal0, sampler_Normal0, uvSplat01.xy), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(TERRAIN_SAMPLE(_Normal1, sampler_Normal0, uvSplat01.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(TERRAIN_SAMPLE(_Normal2, sampler_Normal0, uvSplat23.xy), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(TERRAIN_SAMPLE(_Normal3, sampler_Normal0, uvSplat23.zw), _NormalScale3);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    diffAlbedo[0] = TERRAIN_SAMPLE(_Splat0, sampler_Splat0, uvSplat01.xy);
    diffAlbedo[1] = TERRAIN_SAMPLE(_Splat1, sampler_Splat0, uvSplat01.zw);
    diffAlbedo[2] = TERRAIN_SAMPLE(_Splat2, sampler_Splat0, uvSplat23.xy);
    diffAlbedo[3] = TERRAIN_SAMPLE(_Splat3, sampler_Splat0, uvSplat23.zw);
...
Varyings SplatmapVert(Attributes v)
{
    ...
    half fogFactor = 0;
        ...
        o.fogFactorAndVertexLight.yzw = VertexLighting(Attributes.positionWS, o.normalWS.xyz);
    #else
    ...
    )
{
    ...
    float2 splatUV = (i.uvMainAndLM.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
    ...
    SplatmapMix(i.uvMainAndLM, i.uvSplat01, i.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
    ...
    half3 giColor = ToonGlobalIllumination(brdfData, inputData.bakedGI, aoFactor.indirectAmbientOcclusion, inputData.normalWS, inputData.viewDirectionWS);
    ...
    for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); ++lightIndex) 
    {
        ...
        additionalLightColor += CalculateToonLighting(brdfData, light, inputData.normalWS, inputData.viewDirectionWS, specularOffset);
    }
